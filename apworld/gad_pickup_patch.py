...
# PERMANENT DIVERGENCE FROM THE MAIN REPO'S COPY (documented 2026-08-15,
# found via tools/check_apworld_sync.py): the main repo's version of this
# function imports rsc_utils.build_rsc_record()/inject_rsc_record() to do
# this work. rsc_utils.py is not one of the files duplicated into this AP
# world folder at all (checked -- it's genuinely absent here), so this copy
# hand-inlines an equivalent record-injection routine below instead of
# importing it. That means this is NOT a simple copy/paste target -- if
# rsc_utils.py's injection logic ever changes in the main repo (bugfix,
# format change, etc.), nothing will flag that this function needs the same
# fix, since they're no longer the same code, just parallel implementations
# of the same intent. tools/check_apworld_sync.py will keep reporting this
# file as DRIFT for exactly this reason; that's expected and does not mean
# something is broken -- see CLAUDE.md's 2026-08-15 sync-audit entry.
