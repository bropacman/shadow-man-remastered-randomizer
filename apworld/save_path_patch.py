...
save_path_patch.py
===================
Redirects Shadow Man Remastered's save folder from "saves/" to "ap/" (a
sibling folder next to the vanilla saves directory), so randomizer /
Archipelago playthroughs never share save slots with vanilla saves —
...
Two edits, both IN-PLACE (no new space needed, no pointer redirect):
  ...
  1. The "saves/" string itself (.rdata, VA 0x140704760, 7 bytes incl. the
     null terminator) -> "ap/\0". Confirmed via the Ghidra string dump that
     the very next data item ("g_saveslot") starts immediately at
     VA 0x140704768 -- zero slack beyond the original 8-byte window, which
     is why the replacement had to be short enough to fit in place rather
     than needing scratch space + a LEA displacement patch (the technique
     gad_pickup_patch.py uses for its longer RSC_X_GAD_PICKUP string).
  2. The explicit length argument passed alongside it -- FUN_14033e3b0's own
     ...
     before the LEA that loads the string's address) -- 6 -> 3, matching
     "ap/"'s length.
...
user_activity.dat from:
    <Saved Games>\\Nightdive Studios\\Shadowman EX\\ap\\
instead of the vanilla:
    <Saved Games>\\Nightdive Studios\\Shadowman EX\\saves\\
The "ap" folder is created by the game itself the first time it saves (same
as "saves" already is) -- nothing needs to pre-create it. Vanilla saves are
completely untouched since only the patched exe resolves the different path.
"""
...
NEW_LEAF = "ap/"   # replaces "saves/" -- must stay <= 7 bytes incl. null
                   # (confirmed budget: "g_saveslot" string starts
                   # immediately after, zero slack beyond it)
NEW_LEAF_BYTES = NEW_LEAF.encode("ascii") + b"\x00"
...
LEN_IMM_NEW      = len(NEW_LEAF)

assert len(NEW_LEAF_BYTES) <= STRING_SLOT_LEN, \
    f"NEW_LEAF {NEW_LEAF!r} ({len(NEW_LEAF_BYTES)} bytes) doesn't fit the " \
    f"confirmed {STRING_SLOT_LEN}-byte slot -- would corrupt the adjacent " \
    f"'g_saveslot' string."


def apply_save_path_patch(exe_path: str, *, dry_run: bool = False) -> None:
    """
    Redirect the save-directory leaf from "saves/" to "ap/" in
    ...
    new_string_bytes = NEW_LEAF_BYTES.ljust(STRING_SLOT_LEN, b"\x00")
    ...
    if current_string == new_string_bytes and current_len_imm == LEN_IMM_NEW:
        print(f"  [save_path] Already patched (saves -> {NEW_LEAF!r})")
        return
    ...
    print(f"  [save_path] Save folder redirected: 'saves/' -> {NEW_LEAF!r} "
