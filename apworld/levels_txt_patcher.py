...
# Ported from the standalone randomizer's patchers/levels_txt_patcher.py
# (2026-07-19) with imports converted to package-relative for the AP world.
# Note: this AP world keeps the module flat (no patchers/ subpackage), same
# as the other ported patch modules.
...
def patch_levels_txt(
    ...
    if starting_item_bundles:
        # AP note: starting-item bundles aren't exposed as an AP option, so
        # this branch never runs (starting_item_bundles is always []). The
        # imports stay lazy so a missing STARTING_ITEM_BUNDLES constant can't
        # break module import.
