...
# SL10 + CADEAUX_666(666) gated locations (2026-07-24). Of the 8 raw rows
# whose gate_expr references R.cadeaux_666() (1 "bonus" + 2 "barrel" +
# 5 "enemy"), only this one is ever a real, live AP location to begin with —
# "barrel" is unconditionally skipped in regions.py's _SKIP_CATS and "enemy"
# is unconditionally skipped via _ALWAYS_EXCLUDE above, regardless of any
# option. cadeaux_checkable_count is only 653 (17 cadeaux rows are
# is_verified=False), 13 short of the default fogometers_cadeaux_required
# (666), which made this location unreachable in AP's full-accessibility
# sweep -- confirmed via a live Generate.py repro (FillError: "Could not
# access required locations ... Missing: [Asylum: The Fogometers - Light
# Soul]"). Now gated behind the CadeauxGatedContent option (default off,
# same treatment as barrel/enemy -- excluded entirely) rather than always
# included; when on, __init__.py's generate_early() precollects the
# Cadeaux shortfall as free starting inventory to keep it solvable. See
# regions.py's _build_sub_regions() for where this is actually consulted.
CADEAUX_666_LOCS: frozenset[str] = frozenset({
    "ah4fogom:quest.rsc:0x2742",  # Light Soul
})

FIXED_SOUL_LOCS: list = [
    loc for loc in RAW_LOCATIONS
    if loc.category in ("boss", "true_form")
    # Bug found 2026-07-24: no is_verified filter here (unlike CHECKABLE_LOCS
    # below), so a confirmed-phantom boss/true_form row (tenement:enemies.rsc:
    # 0x00BC, RSC_X_AVERY_MARX, is_verified=FALSE) leaked through and crashed
    # pre_fill() with KeyError -- get_location() raises for any name that was
    # never registered as a real Location, it does not return None. Ported
    # the same exclusion CHECKABLE_LOCS already applies; also hardened
    # pre_fill() itself in __init__.py to skip unregistered locations
    # defensively regardless of the reason.
        ...
        if _enemy_cache is None:
            try:
                ...
                try:
                    ...
                    raise ImportError(
                        "extracted_enemy_locations.py not found — "
                        "true form shuffle requires this file in the world package"
                    )
...
SLOT_ACCEPTS: dict[str, frozenset[str]] = {
    ...
    #
    # "eclipser" was missing from every set below (and had no own key) until
    # 2026-07-19 — confirmed against the standalone randomizer's fill.py,
    # which has always included it here (see its own SLOT_ACCEPTS/comment
    # block: "progression/retractor/accumulator/gad/eclipser → accept
    # progression, retractor, accumulator, gad, eclipser"). The gap meant
    # _is_checkable()/_is_ap_location() excluded all 3 category="eclipser"
    # rows (deadside 0x197A/0x1F1A/0x2352) from CHECKABLE_LOCS/AP_LOCATIONS
    # entirely: the physical Eclipser pickups never became AP locations, so
    # nothing ever detected them being collected (confirmed live — pickup-log
    # entries at their exact CSV coordinates consistently failed to resolve,
    # not because of a detection bug but because these locations had no ap_id
    # to resolve to in the first place). The 3 Eclipser Items themselves
    # were still being created every seed (items.py has no such gap) and
    # placed on some OTHER progression-type slot instead — so games were
    # completable, just with these 3 vanilla slots permanently inert.
...
# the patcher can suppress them from the coverage-validation error count.
# Ported from the standalone repo's fill.py (2026-07-21) — this world's copy
# never had the filter wired into CHECKABLE_LOCS, so is_verified=False rows
# (e.g. the 4 "t4ndgad" cadeaux slots — t4ndgad is a cut sub-zone of Salvage
# with no in-game entrance, see levels_txt_patcher.py) were silently live,
# fillable AP checks.
...
# filter was tried here and reverted the same day — ported from, then
# un-ported to match, the standalone repo's fill.py revert. is_tracked=False
# spans 79% of the CSV (dominated by barrel, already excluded regardless,
# but also 161 cadeaux + 37 real weapon/progression/etc rows) — it's a
# general "does the engine persist this record's collected state" flag, not
# a per-row reliability signal like is_verified. The narrow problem it was
# meant to fix (4 salvage cadeaux rows, 0x552A/0x5602/0x5692/0x56DA) was
# instead fixed directly in the data: those 4 rows now have is_verified=FALSE
# in data/locations.csv, so UNVERIFIED_LOCS above already covers them.
...
# Drift bug found + fixed 2026-07-21 (user report: Govi still showing up,
# uncollectible outside Night, in swampday): "Louisiana Swampland" was
# missing from this set — every other entry matched the standalone's
# canonical fill.py exactly, this one was silently dropped when the AP
# world's copy was ported. Swamp is a real day/night liveside pair
# (swampday/swampnit) with real category=="soul" rows, same Night-locked
# Govi engine quirk as the other 5 liveside regions — see __init__.py's
# Dark Soul retype comment (THIRD FIX, 2026-07-21) for why this set exists
# at all. Per CLAUDE.md: this repo's fill.py is the canonical source and
# the AP world's copy can silently drift behind it — diff, don't assume.
...
REGION_GATES: dict[str, object] = {
    ...
    "Temple of Prophecy (Marcher)" : [
        ...
        # drift this file's own docs warn about). Mirrored from the
        # standalone randomizer's own fill.py.
...
def _shuffle_gates(
    ...
    entrance_shuffle: this world's resolved Deadside portal-file shuffle
             (dict[native_portal_file, dest_portal_file] from __init__.py's
             generate_early(), see regions.py's DEADSIDE_PORTAL_REGION), or
             None when entrance_mode is "off".

             FIXED 2026-08-07 (Jon's report of recurring "Marrow Gates /
             Wasteland" AP accessibility failures under
             entrance_mode=deadside_only, roughly probabilistic — happened
             often but not every seed, even after hardening
             soul_threshold_mode's "random" spacing): every cap above is
             computed against REGION_GATES — a STATIC, VANILLA-topology
             region->gate table. That's correct when entrance_mode is
             "off" (topology matches REGION_GATES exactly). Under
             "deadside_only", each of the 9 Deadside portal files keeps
             its OWN native gate/SL requirement while being reassigned to
             lead to a DIFFERENT region — so "GATE_DEADSIDE_WASTELAND"
             (heavily protected here, capped to SL<=2) might now protect
             some unrelated, already-easy region, while "Deadside -
             Wasteland" itself (home of Baton, a plain UNGATED
             "progression" item several other routes depend on — the
             Cageways/Playrooms/Temple of Prophecy lower-Deadside
             backtracks all include an "Asylum + Baton + Gad2" route) ends
             up behind whichever OTHER portal's native gate got shuffled
             there instead. 3 of the 9 portals (Lavaducts/Fogometers/
             Blood's own native gates) carry a real, up-to-max_sl soul
             requirement with no alternate free route and, once decoupled
             from their vanilla destination this way, no special
             protection at all — roughly 1-in-3 odds per critical
             destination, matching the observed "fails often, not always"
             pattern exactly.

             When provided, the REAL post-shuffle gate for each of the 9
             Deadside-portal-controlled regions is substituted for
             REGION_GATES' vanilla entry before computing protection, so
             it always lands on whichever gate is ACTUALLY standing in
             front of that region this seed — not just its vanilla name.
             No-op (identical to REGION_GATES) when entrance_shuffle is
             None, so entrance_mode="off" behavior is completely
             unchanged.

             Second bug found and fixed the same day, via direct
             simulation before trusting the first fix (see this file's own
             standing "diff against the source, don't assume a port is
             complete" discipline, applied here to a same-file redesign
             instead of a cross-file port): the ORIGINAL swap-based
             mechanism (try to find another shuffleable gate with a low
             enough value and exchange them) can NEVER achieve the
             WASTELAND-tier <=2 cap for a genuinely shuffleable gate,
             because the lowest vanilla SL value among ANY shuffleable
             (non-locked) gate is 3 (GATE_DEADSIDE_LALUNE) — there is no
             donor value <=2 anywhere in that pool to swap in. Verified by
             simulating 3000 random (entrance_shuffle x gate-shuffle)
             combinations: the swap-only version left a real, unprotected
             gate (e.g. GATE_DEADSIDE_FOGOMETERS) sitting at SL7 guarding
             Wasteland in some trials — the exact severe case this fix
             exists to prevent. This also revealed the <=2/<=3/<=5 caps
             were NEVER actually enforced by swapping even before this
             whole fix — WASTELAND/ASYLUM/PATH_3 are always in `locked`,
             so they were never in temp_map and those old constraints were
             silently no-ops the whole time; the caps only ever "worked"
             because those 3 named gates simply never left their own
             already-low vanilla value.

             Real fix: instead of trying to swap a shuffleable gate down
             to a target it structurally can't reach, DYNAMICALLY move it
             into `locked` for this call (pulling it out of the shuffle
             pool entirely, exactly like the original 3 named gates always
             were) and assign it the ORIGINAL role's own vanilla value
             (e.g. GATE_VANILLA_SL["GATE_DEADSIDE_WASTELAND"]) rather than
             its own. This preserves the exact intended early-game
             difficulty for that role regardless of which physical gate
             ended up covering it, and needs no swap/donor value at all.
             Only applies when the real gate is a plain (non-route)
             ALREADY-shuffleable gate — a route-based real gate (e.g. if a
             Cageways/Playrooms-native portal ends up covering one of
             these roles) already has a guaranteed-cheap branch (its own
             front door, itself always forced open) and needs no override;
             confirmed via the same 3000-trial simulation (0 residual
             failures for the route-based Temple-of-Fire/PATH_3 case). A
             real gate that's already one of the OTHER locked named gates
             (e.g. PATH_3 ending up covering the Wasteland role) is left
             as-is too — its own vanilla value is inherently small (1-3)
             either way, a negligible difference from the ideal target,
             confirmed by the same simulation (worst case observed: 3).
    """
    # ── Real (post-entrance-shuffle) region->gate substitution ───────────────
    real_region_gates: dict[str, object] = dict(REGION_GATES)
    if entrance_shuffle:
        from . import regions as _regions_module
        from .access_rules import DEADSIDE_PORTAL_GATE
        # entrance_shuffle maps native_portal_file -> dest_portal_file:
        # walking through native_portal_file's own physical door (guarded
        # by its own DEADSIDE_PORTAL_GATE entry) now leads wherever
        # dest_portal_file vanilla-led. Invert it so, for a region R
        # normally reached via its own native portal NP, we can find which
        # portal ACTUALLY leads there this seed.
        _dest_to_native = {dest: native for native, dest in entrance_shuffle.items()}
        for native_portal, region in _regions_module.DEADSIDE_PORTAL_REGION.items():
            real_portal = _dest_to_native.get(native_portal)
            if real_portal is not None and region in real_region_gates:
                real_region_gates[region] = DEADSIDE_PORTAL_GATE[real_portal]

    def _gate_tokens(gate_spec) -> set[str]:
        """Flatten a REGION_GATES-style value (a plain gate id, or a list
        of alternate routes each a list of tokens) into every gate-id-like
        token it references. Non-gate tokens (e.g. "BATON", "GAD2_WALK")
        are harmless here — they're simply never keys in temp_map, so
        every later lookup against them is a no-op, same tolerance
        REGION_GATES' route lists already relied on before this fix."""
    ...
    starting_gates = (
        _gate_tokens(real_region_gates.get("Deadside - Wasteland", "GATE_DEADSIDE_WASTELAND")) |
        _gate_tokens(real_region_gates.get("Asylum: Gateways", "GATE_DEADSIDE_ASYLUM")) |
        _gate_tokens(real_region_gates.get("Temple of Fire (Toucher)", "GATE_DEADSIDE_PATH_3"))
    )

    # ── Dynamic-lock overrides for the 3 critical roles ──────────────────────
    # See this function's own docstring above for the full derivation. Only
    # fires for a role whose REAL gate is a plain (non-route) id that isn't
    # already locked — that's the one case the swap mechanism below can
    ...
    # already-applied constraints. The old constraints 2-4 (starting_gates
    # SL3 / WASTELAND SL2 / PATH_3 SL5, all swap-based) were removed here —
    # see the dynamic_overrides block above for why swapping can't achieve
    # them and what replaces them.
    constraints = []
...
def build_gate_rules(gate_remap: dict[str, int] | None = None) -> dict:
    ...
    # Lightweight fake region — no AP BaseClasses involved
    class _FakeRegion:
    ...
    # Temporarily replace Region with our fake in regions_module
    from . import regions as regions_module
    from .access_rules import GATE_VANILLA_SL as _GVS

    original_connect = regions_module._connect
    original_region_class = regions_module.Region

    # Patch Region class used in create_regions
    regions_module.Region = _FakeRegion

    def recording_connect(source, target, rule=None, name=None):
        # name= added 2026-07-20 for entrance naming in regions.py's real
        # _connect(); this dry-run stand-in doesn't need it, just accept
        # and ignore so create_regions() doesn't choke on the kwarg.
        parent_map[target.name] = (source.name, rule or (lambda state: True))

    regions_module._connect = recording_connect

    # Fake location factory — just needs to not crash
    class _FakeLoc:
        def __init__(self, player, name, raw, parent):
            self.name = name
            self.raw  = raw

    regions_module.create_regions(
        world,
        PLAYER,
        gate_values      = gate_remap or dict(_GVS),
        location_factory = _FakeLoc,
    )

    regions_module._connect = original_connect
    regions_module.Region   = original_region_class
    ...
    for name in list(parent_map.keys()) + list(set(src for src, _ in parent_map.values())):
        get_rule(name)
...
class FakeState:
    ...
    def count(self, item: str, player: int) -> int:
        if item in ("_souls", "Dark Soul"):       return self.soul_count
        if item in ("_retractors", "Retractor"):  return self.retractor_count
        if item in ("_cadeaux",):                 return self.cadeaux_count
...
def assumed_fill(
    ...
    remaining_locs = [
        ...
        if loc.loc_key not in placement
        and loc.category in FILLER_SLOT_CATS | {"soul"}
    ]
    ...
    if verbose or not ok:
        ...
        if not goal_met:
            ...
            for label, met in [
                ...
                ("5x Retractor",        final_state.count("Retractor", PLAYER) >= 5),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gate-preset",
                        choices=["story", "easy", "medium", "hard", "chaos"],
                        default=None,
